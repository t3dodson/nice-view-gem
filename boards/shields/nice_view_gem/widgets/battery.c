...
static void draw_level(lv_obj_t *canvas, const struct status_state *state) {
    ...
    canvas_draw_text(canvas, 0 + BOLT_WIDTH + (2 * BOLT_MARGIN) + TOP_LEFT_MARGIN, 0 + TOP_LEFT_MARGIN, 42, &label_right_dsc, text);
}
...
static void draw_charging_level(lv_obj_t *canvas, const struct status_state *state) {
    ...
    canvas_draw_img(canvas, BOLT_MARGIN + TOP_LEFT_MARGIN, BOLT_MARGIN + TOP_LEFT_MARGIN, &bolt, &img_dsc);
}
